...
class ImgurPopulator:
	...
	def _store_rows_to_db(self, images):
		if self.config['database'].get('unix_socket', None):
			cnx = mysql.connector.connect(user=self.config['database']['user'], database=self.config['database']['schema'], password=self.config['database']['password'], unix_socket=self.config['database']['unix_socket'])
		else:
			cnx = mysql.connector.connect(user=self.config['database']['user'], database=self.config['database']['schema'], password=self.config['database']['password'], host=self.config['database']['host'], port=self.config['database']['port'])
